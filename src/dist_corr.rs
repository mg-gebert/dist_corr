...
pub fn dist_corr(v1: &[f64], v2: &[f64]) -> Result<f64, Box<dyn Error>> {
    let v1_len = v1.len();
    ...
    let (dist_var_v1, dist_var_v2) = join(
        || {
            ...
            dist_var_helper(&v1_shuffled, &grand_means_v1, v1_len as f64)
        },
        || {
            grand_means(&v2_sorted, None, &mut grand_means_v2, v1_len);
            dist_var_helper(&v2_sorted, &grand_means_v2, v1_len as f64)
        },
    );
...
pub fn dist_cov(v1: &[f64], v2: &[f64]) -> Result<f64, Box<dyn Error>> {
    let v1_len = v1.len();
...
pub fn dist_var(v: &[f64]) -> f64 {
    ...
    dist_var_helper(v, &grand_means_v_sorted, v_len as f64)
}

pub fn dist_cov_helper(
    v1: &[f64],
    v2: &[f64],
    grand_mean_v1: &[f64],
    grand_mean_v2: &[f64],
...
pub fn dist_var_helper(v: &[f64], grand_means: &[f64], len: f64) -> f64 {
