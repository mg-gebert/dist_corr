...
pub fn dist_cov_naive_exp(v_1: &[f64], v_2: &[f64]) -> f64 {
    ...
    let dist_scalar_avg = v_1
        ...
        .zip(v_2.par_iter())
        .map(|(a1, b1)| {
            let data1i = v_1.iter().map(|a2| (-(a1 - a2).abs()).exp()).sum::<f64>();
            let data2i = v_2.iter().map(|b2| (-(b1 - b2).abs()).exp()).sum::<f64>();
            data1i * data2i
        })
        .sum::<f64>()
        / (data_length * data_length * data_length);
    ...
    v_1.iter().zip(v_2.iter()).for_each(|(a1, b1)| {
        mean_a += v_1.iter().map(|a2| (-(a1 - a2).abs()).exp()).sum::<f64>();
        mean_b += v_2.iter().map(|b2| (-(b1 - b2).abs()).exp()).sum::<f64>();
    });
