...
Small and fast Rust utilities for computing distance correlation and distance covariance between pairs of numeric vectors — with optimized implementations for binary (0/1) data.

- Distance correlation: a dependence measure between two random vectors that is zero if and only if the vectors are independent. Returns a value in [0, 1]. It was introduced in the seminal paper: 
...
This crate exposes two small ergonomic types:
- `DistCorrelation` — compute distance correlation
- `DistCovariance` — compute distance covariance and distance variance
...
### Two arbitrary real-valued vectors
...
1. `O(n log(n))` if one vector is binary but considerably faster than the vanilla implementation above. 
...
The implementation of the above is considerably faster than calling `DistCovariance::compute(v, v)` when the input `v` is non-binary.
